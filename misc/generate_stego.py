...
def precover_to_cover(
    ...
    with torch.no_grad():
        ...
        for file in tqdm(files_to_convert):
            ...
            torch.save(compressed_img_t, path + "/" + str(p).replace(".","_") + ".pt")
            if p > 0.:
                info = f"p = {p} H3(p) = {H(p)}\n"
...
if __name__ == "__main__":
    ...
    parser.add_argument("-p","--probability", nargs="+",type=float,default=.0,help="Half insertion rate",required=True)

    args = parser.parse_args()
    if not os.path.exists(args.output):
        os.makedirs(args.output,exist_ok=True)
    for p in args.probability:
        precover_to_cover(args.name, args.output, args.model, p, args.begin, args.end)